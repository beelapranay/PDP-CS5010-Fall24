...
public class ImageModel implements ImageModelInterface {
  ...
  private static class ImageData {
    ...
    ImageData(RGB[][] originalArray, RGB[][] transformedArray, int width,
              int height, int splitPoint) {
   ...
   */
  private ImageData getImageData(String imageName, int splitPercentage)
  ...
  public void blurOrSharpen(String imageName, String destinationImageName,
    ...
    switch (operationName) {
      case "blur":
        kernel = blurKernel;
        kernelSize = blurKernel.length;
        break;
      case "sharpen":
        kernel = sharpenKernel;
        kernelSize = sharpenKernel.length;
        break;
      default:
        throw new NoSuchElementException("Invalid operation: " + operationName);
    }
    ...
    for (int y = 0; y < imageData.height; y++) {
      for (int x = 0; x < imageData.width; x++) {
        if (x < imageData.splitPoint) {
          if (y < offset || y >= imageData.height - offset || x < offset
                  || x >= imageData.width - offset) {
            imageData.transformedArray[y][x] = imageData.originalArray[y][x];
          } else {
            double[] newRGB = applyKernel(imageData.originalArray, x, y, kernel, kernelSize);
            imageData.transformedArray[y][x] = new RGB((int) newRGB[0],
                    (int) newRGB[1], (int) newRGB[2]);
          }
        } else {
          imageData.transformedArray[y][x] = imageData.originalArray[y][x];
        }
  ...
  private double[] applyKernel(RGB[][] array, int x, int y, double[][] kernel, int kernelSize) {
    double red = 0;
    double green = 0;
    double blue = 0;
  ...
  public void colorCorrect(String imageName, String destinationImageName, int splitPercentage) {
    ...
    RGB[][] correctedImage = applyColorCorrection(pixelArray, redOffset,
            greenOffset, blueOffset, splitPercentage);
  ...
  public void levelsAdjust(String imageName, String destImageName, int b, int m, int w,
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (x < imageData.splitPoint) {
          int adjustedRed = applyQuadraticLevelsAdjustment(image[y][x].red, aCoef, bCoef, cCoef);
          int adjustedGreen =
                  applyQuadraticLevelsAdjustment(image[y][x].green, aCoef, bCoef, cCoef);
          int adjustedBlue = applyQuadraticLevelsAdjustment(image[y][x].blue, aCoef, bCoef, cCoef);
          adjustedImage[y][x] = new RGB(adjustedRed, adjustedGreen, adjustedBlue);
        } else {
          adjustedImage[y][x] = image[y][x];
        }
      }
    }
  ...
  private RGB[][] applyColorCorrection(RGB[][] pixelArray, int redOffset, int greenOffset,
                                       int blueOffset, int splitPercentage) {
    int height = pixelArray.length;
    int width = pixelArray[0].length;
   ...
   */
  @Override
  public void downscaleImage(String sourceImageName, String destinationImageName,
                             int targetWidth, int targetHeight) {
    RGB[][] sourcePixels = imageController.getImages().get(sourceImageName);

    if (sourcePixels == null) {
      throw new NoSuchElementException("Source image not found: " + sourceImageName);
    }

    int sourceWidth = sourcePixels[0].length;
    int sourceHeight = sourcePixels.length;

    RGB[][] targetPixels = new RGB[targetHeight][targetWidth];
    ...
    for (int y = 0; y < targetHeight; y++) {
      for (int x = 0; x < targetWidth; x++) {
        ...
        RGB cA = sourcePixels[y1][x1];
        RGB cB = sourcePixels[y1][x2];
        RGB cC = sourcePixels[y2][x1];
        RGB cD = sourcePixels[y2][x2];

        int red = (int) ((cA.red * (1 - xDiff) + cB.red * xDiff) * (1 - yDiff)
                + (cC.red * (1 - xDiff) + cD.red * xDiff) * yDiff);
        int green = (int) ((cA.green * (1 - xDiff) + cB.green * xDiff) * (1 - yDiff)
                + (cC.green * (1 - xDiff) + cD.green * xDiff) * yDiff);
        int blue = (int) ((cA.blue * (1 - xDiff) + cB.blue * xDiff) * (1 - yDiff)
                + (cC.blue * (1 - xDiff) + cD.blue * xDiff) * yDiff);

        targetPixels[y][x] = new RGB(clamp(red), clamp(green), clamp(blue));
      }
    }

    imageController.getImages().put(destinationImageName, targetPixels);
  }
