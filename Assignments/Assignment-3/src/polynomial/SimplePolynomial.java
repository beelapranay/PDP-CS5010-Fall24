...
public class SimplePolynomial implements Polynomial {
  ...
  public boolean equals(Object other) {
    ...
    if (other instanceof SparsePolynomial) {
      if (((SparsePolynomial) other).getPolynomialTerms().size() != this.getSize()) {
        return false;
      }

      for (Terms term : ((SparsePolynomial) other).getPolynomialTerms()) {

        if (term.getCoefficient() != this.coefficients[term.getPower()]) {
          return false;
        }
    ...
    for (int i = 0; i <= this.getDegree(); i++) {
      if (this.getCoefficient(i) != otherPolynomial.getCoefficient(i)) {
        return false;
      }
    }
  ...
  public int hashCode() {
    ...
    for (int i = 0; i <= getDegree(); i++) {
      if (coefficients[i] != 0) {
        result = 37 * result + coefficients[i];
        result = 37 * result + i;
      }
    }
    return result;
  ...
  /**
   * Returns the size of this polynomials array.
   *
