...
/**
 * Command to perform sharpen operation, with optional split view or masking.
 */
public class SharpenCommand implements Command {
  ...
  /**
   * Constructs a sharpen command with the given parameters.
   *
   ...
   */
  public SharpenCommand(String[] tokens, AdvancedImageProcessor imageProcessor,
                        ImageModelInterface model) {

    if (tokens.length < 3 || tokens.length > 5) {
      throw new IllegalArgumentException("Usage: " +
              "sharpen sourceImage [maskImage] destImage [split p]");
    }
  ...
  /**
   * Executes the sharpen command on the given image.
   */
  @Override
  public void execute() {
    ...
    if (maskImageName != null) {
      ...
      result = imageProcessor.applyMask(image, maskImage, "sharpen");
    } else if (isSplit) {
      result = imageProcessor.splitView(image, "sharpen", splitPercentage);
    } else {
      result = imageProcessor.sharpen(image);
    }
