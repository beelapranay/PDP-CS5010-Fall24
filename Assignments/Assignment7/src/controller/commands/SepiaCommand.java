...
/**
 * Command to perform sepia tone operation, with optional split view or masking.
 */
public class SepiaCommand implements Command {
  ...
  /**
   * Constructs a sepia tone command with the given parameters.
   *
   ...
   */
  public SepiaCommand(String[] tokens, AdvancedImageProcessor imageProcessor,
                      ImageModelInterface model) {

    if (tokens.length < 3 || tokens.length > 5) {
      throw new IllegalArgumentException("Usage: " +
              "sepia sourceImage [maskImage] destImage [split p]");
    }
  ...
  /**
   * Executes the sepia command on the given image.
   */
  @Override
  public void execute() {
    ...
    if (maskImageName != null) {
      ...
      result = imageProcessor.applyMask(image, maskImage, "sepia");
    } else if (isSplit) {
      result = imageProcessor.splitView(image, "sepia", splitPercentage);
    } else {
      result = imageProcessor.sepia(image);
    }
